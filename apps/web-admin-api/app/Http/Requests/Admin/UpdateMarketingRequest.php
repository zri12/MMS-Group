...
class UpdateMarketingRequest extends FormRequest
{
    ...
    {
        /** @var MarketingProfile $marketing */
        $marketing = $this->route('marketing');

        return [
            'name' => ['required', 'string', 'max:150'],
            'username' => ['required', 'string', 'max:100', 'regex:/^[a-z0-9._-]+$/', Rule::unique('users', 'username')->ignore($marketing->user_id)],
            'email' => ['nullable', 'string', 'email', 'max:150', Rule::unique('users', 'email')->ignore($marketing->user_id)],
            'code' => ['required', 'string', 'max:10', Rule::unique('marketing_profiles', 'code')->ignore($marketing->id)],
